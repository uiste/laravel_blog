    ...
    <form action="{{url('admin/category/'.$cate_info->cate_id)}}" method="post">
        ...
        <table class="add_tab">
            ...
            <tr>
                ...
                <td>
                    <select name="cate_pid">
                        ...
                        @foreach($data as $v)
                            <option value="{{$v->cate_id}}" @if($v->cate_id==$cate_info->cate_pid) selected @endif >{{$v->cate_name}}</option>
                        @endforeach
                    </select>
                </td>
            </tr>
            ...
            <tr>
                <th>描述：</th>
                <td>
                    <textarea class="lg" name="cate_description">{{$cate_info->cate_description}}</textarea>
                    <p>标题可以写30个字</p>
                </td>
            </tr>
            <tr>
